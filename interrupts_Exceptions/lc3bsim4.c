...
void cycle() { 
    ...
    printf("Cycle 300, so execute timer 1 interrupt\n");
...
void initialize(char *argv[], int num_prog_files) { 
    ...
    CURRENT_LATCHES.UserR6 = 0xFE00; /* Initial value of user stack pointer*/
    CURRENT_LATCHES.ProgramStatusRegister = 0x8002; /* Initial value of PSR in user mode and NZP = 010*/
...
void eval_bus_drivers() {
    ...
    if(getGATER6){
        if(getLDR6){
            ...
            if(getR6MUX == NO){
                NEXT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS[OFF6] + nextLocation;
                CURRENT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS + nextLocation;
                BUS = Low16bits(NEXT_LATCHES.REGS[OFF6]);
            }
        }else{
    ...
    if(getGateMARMUX){
        if(getMARMUX == NO){
            ...
            if((first_address + second_address) < initial && (NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV){
                either = TRUE;
                excpt = TRUE;
   ...
   */   
    ...
    if(getGATEALU == YES){
        ...
        if(getldreg == YES){
            ...
            else if(GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
                NEXT_LATCHES.REGS[7] = Low16bits(BUS);
            }
            CTLSIG[4] = YES;
        }
        if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            ...
            //except_prot.asm
            if((NEXT_LATCHES.ProgramStatusRegister & 0x8000)==0x8000 && NEXT_LATCHES.MAR < initial && (NEXT_LATCHES.IR & HighWord) != HighWord){
                excpt = TRUE;
                either = TRUE;
                NEXT_LATCHES.EXCV = BREN;
                NEXT_LATCHES.STATE_NUMBER = 36;
        ...
        if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            ...
            if((NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV)== PSR_PRIV && NEXT_LATCHES.MAR < initial && (NEXT_LATCHES.IR & HighWord)!= highWordModify){
                excpt = TRUE;
                either = TRUE;
...
void latch_datapath_values() {
    ...
    if(GetLD_PC(CURRENT_LATCHES.MICROINSTRUCTION) == YES && CTLSIG[6] == NO){
        ...
        if(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            int register_num = (CURRENT_LATCHES.IR & 0x01C0) >> OFF6;
            NEXT_LATCHES.PC = CURRENT_LATCHES.REGS[register_num];
            if(NEXT_LATCHES.PC < initial && (NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV){
                excpt = TRUE;
                either = TRUE;
                NEXT_LATCHES.EXCV = BREN; 
                NEXT_LATCHES.STATE_NUMBER = 36;
