...
int GetLDVEC(int *x)     { return(x[LDVEC]);}
int GetLDPSR(int *x)        { return(x[LDPSR]);}
int GetLDR6(int *x)         { return(x[LDR6]);}
int GetLDCHANGE(int *x)     { return(x[LDCHANGE]);}
int GetLDIVT(int *x)        { return(x[LDIVT]);}
int GetLDCURRENT(int *x)    { return(x[LDCURRENT]);}
int GetLDREG_VA(int *x)     { return(x[LDREG_VA]);}
int GetLDREG_PA(int *x)     { return(x[LDREG_PA]);}
int GetLDVA(int *x)         { return(x[LDVA]);}
...
int GetSWITCH_SSP(int *x)        { return(x[SWITCH_SSP]);}
int GetSWITCH_USP(int *x)        { return(x[SWITCH_USP]);}
int GetR6MUX(int *x)        { return(x[R6MUX]);}
int GetCLR_PSR(int *x)       { return(x[CLR_PSR]);}
int GetPG_BITS(int *x)       { return(x[PG_BITS]);}
...
int ProgramStatusRegister; // initial value of Program Status Register
int UserR6;
...
#define CYCLE_INT 300
#define TIMER1_INT 0x00000001
void cycle() {                                                
  // check to see if the cycle count is equal to 300
    if(CYCLE_COUNT == CYCLE_INT) {
    // timer interrupt occurs:
    NEXT_LATCHES.INTV = TIMER1_INT;
  }
...
void eval_micro_sequencer() {
  ...
  if(IRD_Value == 1){
    ...
    while(iterate < CONTROL_STORE_BITS){
   ...
   // where the IRD is 0
    ...
    // microinstruction
        ...
        if((JSR_R & 0x1) == 1){
            ...
            int iterate_copy = 0;
            while(iterate_copy < CONTROL_STORE_BITS){
        ...
        if(CRBEN == 0){
            NEXT_LATCHES.STATE_NUMBER = GetJ(CURRENT_LATCHES.MICROINSTRUCTION);
            int index = 0;
            while(index < CONTROL_STORE_BITS){
            ...
            int parse = 0;
            while(parse < CONTROL_STORE_BITS){
...
void updateBeforeFourth(){
    int control_signals = 7;
    for(int i = 0; i < control_signals; i++){
        if(CTLSIG[i] == NO){
            CTLSIG[i] = YES; 
        }
        mem_cycle++;
            ...
            switch(r_w){
                case 0:{
                    if((CURRENT_LATCHES.MAR & READYBIT) == YES){
                        ...
                        NEXT_LATCHES.MDR = Low16bits((MEMORY[(CURRENT_LATCHES.MAR & MDR0) >> YES][1] << shiftby8));
                        NEXT_LATCHES.MDR = NEXT_LATCHES.MDR + (MEMORY[(CURRENT_LATCHES.MAR & MDR0) >> YES][0] & LowWord);
                        CTLSIG[1] = YES;
                    }else{
                        NEXT_LATCHES.MDR = Low16bits((MEMORY[CURRENT_LATCHES.MAR >> YES][1] << shiftby8));
                        NEXT_LATCHES.MDR = NEXT_LATCHES.MDR + (MEMORY[CURRENT_LATCHES.MAR >> YES][0] & LowWord);
                        CTLSIG[1] = YES;
                    }
                    break;
                }
                case 1:{
                    int wSize = GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION);
                    if(wSize == YES){
                        MEMORY[CURRENT_LATCHES.MAR >> YES][0] = Low16bits(CURRENT_LATCHES.MDR & LowWord);
                        MEMORY[CURRENT_LATCHES.MAR >> YES][1] = Low16bits((CURRENT_LATCHES.MDR & HighWord) >> shiftby8);
                    }
...
void getALUK(int source1, int source2){
    int operation; 
    int modeALU = GetALUK(CURRENT_LATCHES.MICROINSTRUCTION);
    if(modeALU == ADD){
...
void eval_busGateMARMUX(){
   // int* curr_u = CURRENT_LATCHES.MICROINSTRUCTION;
    if(GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == NO){
        ...
        int hold_value = Low16bits(Low16Addr1 + Low16Addr2);
        BUS = hold_value;
    }
}
...
int getResultADDR2(){
    ...
    if(muxADDR2 == PCoffset11){
        total = SEXT(CURRENT_LATCHES.IR & MAXOFF11, OFF12);
    }
    else if(muxADDR2 == PCoffset9){
        total = SEXT(CURRENT_LATCHES.IR & MAXOFF9, OFF9);
    }
    else if(muxADDR2 == offset6){
        total = SEXT(CURRENT_LATCHES.IR & MAXOFF6, OFF6);
    }
...
void eval_busGateMDR(){
    ...
    if((CURRENT_LATCHES.MAR & READYBIT) == YES){
        ...
        BUS = extension_of;
            ...
            int getR6 = GetLDR6(CURRENT_LATCHES.MICROINSTRUCTION);
            int getR6mux = GetR6MUX(CURRENT_LATCHES.MICROINSTRUCTION);
            if(getR6){
                if(getR6mux == NO){
                    NEXT_LATCHES.REGS[6] = CURRENT_LATCHES.REGS[6] + nextLocation;
                    CURRENT_LATCHES.REGS[6] = CURRENT_LATCHES.REGS[6] + nextLocation;
                }
            }
...
void setIVTToYes(){
    NEXT_LATCHES.IVT == YES;
}

////////////////////////////////////////////////////////////////
// lab 4 code

#define PSR_STATE 0x27
#define excpt_STATE 0x24
#define initial 0x3000
#define shiftBy8 8
#define timerINT 0x0200
...
///////////////////////////////////////////////////////////////
// lab 5 code
#define rightShift9 9
#define MASK15_9 0xFE00
#define MAXFRAME 23
...
void eval_bus_drivers() {
    ...
    int getgateVEC = GetGATE_VEC(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getgateVEC){
        if(intpt == YES){
            BUS = Low16bits(timerINT | CURRENT_LATCHES.INTV);
            NEXT_LATCHES.INTV = NO;
            intpt = FALSE;
        }else if(excpt == YES){
            BUS = Low16bits(timerINT | CURRENT_LATCHES.EXCV);
            NEXT_LATCHES.EXCV = NO;
            excpt = FALSE;
        }
    }
    int getGATEPSR = GetGATE_PSR(CURRENT_LATCHES.MICROINSTRUCTION);
    int getclrPSR = GetCLR_PSR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getGATEPSR){
        if(getclrPSR){
            if((CURRENT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV){
            ...
            NEXT_LATCHES.ProgramStatusRegister = NEXT_LATCHES.ProgramStatusRegister & nextPSR;
        }
        int shift1 = GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION);
        if(shift1){
            NEXT_LATCHES.EXCV = CURRENT_LATCHES.EXCV << YES;
            NEXT_LATCHES.INTV = CURRENT_LATCHES.INTV << YES;
        }
        BUS = Low16bits(CURRENT_LATCHES.ProgramStatusRegister);
    }
    int getGATER6 = GetGATE_R6(CURRENT_LATCHES.MICROINSTRUCTION);
    int getLDR6 = GetLDR6(CURRENT_LATCHES.MICROINSTRUCTION);
    int getR6MUX = GetR6MUX(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getGATER6){
        if(getLDR6){
            if(getR6MUX){
                NEXT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS[OFF6] - nextLocation;
                CURRENT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS[OFF6] - nextLocation;
                BUS = Low16bits(NEXT_LATCHES.REGS[OFF6]);
            }
            if(getR6MUX == NO){
                NEXT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS[OFF6] + nextLocation;
                CURRENT_LATCHES.REGS[OFF6] = CURRENT_LATCHES.REGS[OFF6] + nextLocation;
            ...
            BUS = Low16bits(CURRENT_LATCHES.REGS[OFF6]);
        }
    }
    ...
    if(getGateMARMUX){
        if(getMARMUX == NO){
            ...
            imm = CURRENT_LATCHES.IR & LowWord;
            imm = imm << YES;
            BUS = Low16bits(imm);
            ...
            short second_address = 0;
            short first_address = 0;
            ...
            switch(address2MUX){
                ...
                case READYBIT:{
                    second_address = CURRENT_LATCHES.IR & MAXOFF6;
                    second_address = SEXT(second_address, OFF6);
                    second_address = Low16bits(second_address);
                    break; 
                }
                case BREN:{
                    second_address = CURRENT_LATCHES.IR & MAXOFF9;
                    second_address = SEXT(second_address, OFF9);
                    second_address = Low16bits(second_address);
                    break;
                }
                case ADDRMODE:{
                    second_address = CURRENT_LATCHES.IR & MAXOFF11;
                    second_address = SEXT(second_address, OFF12);
                    second_address = Low16bits(second_address);
                    break;
                }
            }
            int leftshift = GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION);
            if(leftshift == YES){
                second_address = second_address << YES;
            }
            int address1MUX = GetADDR1MUX(CURRENT_LATCHES.MICROINSTRUCTION);
            switch(address1MUX){
                case 0:{
                    first_address = CURRENT_LATCHES.PC;
                    first_address = Low16bits(first_address);
                    break;
                }
                case READYBIT:{
                    int sr1mux = GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION);
                    if(sr1mux){
                        int register_num = CURRENT_LATCHES.IR & R7;
                        register_num = register_num >> OFF6;
                        first_address = CURRENT_LATCHES.REGS[register_num];
                        first_address = Low16bits(first_address);
                        break;
                    }
                }
            }
            if((((first_address + second_address) & MASK15_9) >> rightShift9) <= MAXFRAME && (NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV && CURRENT_LATCHES.IVT == NO){
                either = TRUE;
                excpt = TRUE;
                NEXT_LATCHES.EXCV = BREN;
            ...
            else if(((first_address + second_address) & READYBIT) == YES && GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
                either = TRUE;
                excpt = TRUE;
                NEXT_LATCHES.EXCV = ADDRMODE;
                NEXT_LATCHES.STATE_NUMBER = 36;
                int parse = 0;
                while(parse < CONTROL_STORE_BITS){
                    NEXT_LATCHES.MICROINSTRUCTION[parse] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][parse];
            ...
            BUS = Low16bits(first_address + second_address);
        }
        ...
        if(GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION)){
            firstReg = (CURRENT_LATCHES.IR & R7) >> OFF6;
            if((CURRENT_LATCHES.IR & 0x0020) == NO){
                secondReg = (CURRENT_LATCHES.IR & 0x0007);
                int total = 0;
                switch(modeALU){
                    case 0:{
                        total = CURRENT_LATCHES.REGS[firstReg] + CURRENT_LATCHES.REGS[secondReg];
                        total = Low16bits(total);
                        BUS = total;
                        break;
                    }
                    case READYBIT:{
                ...
                imm = (CURRENT_LATCHES.IR & IMMOFF5);
                imm = SEXT(imm, OFF5);
                int total = 0;
                    switch(modeALU){
                        case 0:{
                            total = CURRENT_LATCHES.REGS[firstReg] + imm;
                            total = Low16bits(total);
                            BUS = total;
                            break;
                        }
            ...
            firstReg = (CURRENT_LATCHES.IR & CondCode) >> OFF9;
            BUS = CURRENT_LATCHES.REGS[firstReg];
        }
        ...
        if((CURRENT_LATCHES.MAR & READYBIT) == YES){
            ...
            if(data_size == NO){
                int hold_value = SEXT((CURRENT_LATCHES.MDR & HighWord) >> shiftby8, shiftby8);
                BUS = Low16bits(hold_value);
            }
        }else{
            int data_size = GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION);
            if(data_size == NO){
                int hold_value = SEXT(CURRENT_LATCHES.MDR & LowWord, shiftby8);
                BUS = Low16bits(hold_value);
            }else{
                int getR6 = GetLDR6(CURRENT_LATCHES.MICROINSTRUCTION);
                int getR6MUX = GetR6MUX(CURRENT_LATCHES.MICROINSTRUCTION);
        ...
        unsigned char pc_mux = GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION);
        int first_address = 0;
        int second_address = 0;
        if(pc_mux == 2 && GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) != YES){
            int getADDR2MUX = GetADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION);
            switch(getADDR2MUX){
                case 0:{
                    second_address = 0;
                    break;
                ...
                case ADDRMODE:{
                    second_address = CURRENT_LATCHES.IR & MAXOFF11;
                    second_address = SEXT(second_address, 11);
                    second_address = Low16bits(second_address);
                    break;
                }
            }
            int getLSHF1 = GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION);
            if(getLSHF1 == YES){
                second_address = second_address << YES;
            }
        ...
        unsigned short type_shift = CURRENT_LATCHES.IR & 0x0030;
        type_shift = type_shift >> 4;
        unsigned char imm = CURRENT_LATCHES.IR & 0x000F;
        int firstReg = CURRENT_LATCHES.IR & R7;
        firstReg = firstReg >> 6;
        switch(type_shift){
            case NO:{
                short val = CURRENT_LATCHES.REGS[firstReg] << imm;
                val = Low16bits(val);
                BUS = val;
                break;
            }
            case READYBIT:{
                unsigned short val = CURRENT_LATCHES.REGS[firstReg] >> imm;
                val = Low16bits(val);
                BUS = val;
                break;
            }
            case ADDRMODE:{
                short val = SEXT(CURRENT_LATCHES.REGS[firstReg],16) >> imm;
                val = Low16bits(val);
                BUS = val;
                BUS = Low16bits(BUS);
                break;
...
#define next_state 37
#define last_state 52
////////////////////////////////////////////////////////////////
// lab 3 code
void handleALUGate() {
...
#define ADDREXCV 0x0004
#define highWordModify 0xF000
#define saveHighBit 0xA000
#define saveHighBit2 0xB000
void handlePCGate() {
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == YES) {
        ...
        // except_prot.asm
        if((NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV && NEXT_LATCHES.MAR < initial && (NEXT_LATCHES.IR & HighWord) != HighWord){
            excpt = YES;
            either = YES;
            NEXT_LATCHES.EXCV = BREN;
            NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
            int parse = 0;
            while(parse < CONTROL_STORE_BITS){
                NEXT_LATCHES.MICROINSTRUCTION[parse] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][parse];
                parse++;
    ...
    if(getLDMDR && CTLSIG[1] != YES){
        NEXT_LATCHES.MDR = Low16bits(BUS);
    }
}
#define highbit 0x8000
void handleMDRGate() {
    int getLDPSR = GetLDPSR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getLDPSR == YES){
        NEXT_LATCHES.ProgramStatusRegister = Low16bits(BUS);
        if((NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV){
            NEXT_LATCHES.STATE_NUMBER = last_state;
            int parse = 0;
            while(parse < CONTROL_STORE_BITS){
                NEXT_LATCHES.MICROINSTRUCTION[parse] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][parse];
                parse++;
    ...
    if(GetLD_IR(CURRENT_LATCHES.MICROINSTRUCTION) == YES) {
        ...
        CTLSIG[2] = YES;
        if((NEXT_LATCHES.IR & highWordModify) == saveHighBit || (NEXT_LATCHES.IR & highWordModify) == saveHighBit2){
            excpt = YES;
            either = YES;
            NEXT_LATCHES.EXCV = ADDREXCV;
            NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
            int parse = 0;
            while(parse < CONTROL_STORE_BITS){
                NEXT_LATCHES.MICROINSTRUCTION[parse] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][parse];
...
void handleMARMUXGate() {
    ...
    if(getLD_MAR == YES) {
        NEXT_LATCHES.MAR = Low16bits(BUS);
        CTLSIG[0] = YES;
        if((NEXT_LATCHES.ProgramStatusRegister & highbit) == highbit && NEXT_LATCHES.MAR < initial && (NEXT_LATCHES.IR & HighWord) != highWordModify){
            excpt = YES;
            either = YES;
            NEXT_LATCHES.EXCV = BREN;
            NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
...
void handleVECGATE(){
    int getGATEVEC = GetGATE_VEC(CURRENT_LATCHES.MICROINSTRUCTION);
    int getLDMAR = GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getGATEVEC == YES){
        if(getLDMAR){
            NEXT_LATCHES.MAR = Low16bits(BUS);
        }
...
void handleR6GATE(){
    int getR6GATE = GetGATE_R6(CURRENT_LATCHES.MICROINSTRUCTION);
    int getLD_mar = GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getR6GATE == YES){
        if(getLD_mar){
            NEXT_LATCHES.MAR = Low16bits(BUS);
...
#define Bounds 0x7000

// handles the translation for pa to va
// must add to make sure the frame is less because we cannot go more than 23 frames as this will
// result in an exception and access physical memory and not virtual memory
void VA_PATranslaton(){
    if((NEXT_LATCHES.MAR & MASK15_9) >> rightShift9 < MAXFRAME){
        NEXT_LATCHES.STATE_NUMBER = firstVAPAState;
        int parse = 0;
        while(parse < CONTROL_STORE_BITS){
            NEXT_LATCHES.MICROINSTRUCTION[parse] = CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER][parse];
            parse++;
    ...
    if((NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV && ((NEXT_LATCHES.MAR & MASK15_9) >>  rightShift9) <= MAXFRAME && (NEXT_LATCHES.IR & HighWord) != HighWord && CURRENT_LATCHES.IVT == NO){
        excpt = YES;
        either = YES;
        NEXT_LATCHES.EXCV = BREN;
        NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
...
void continued_translation(){
    if((CURRENT_LATCHES.IR & highWordModify) == Bounds || (CURRENT_LATCHES.IR & highWordModify) == initial){

                NEXT_LATCHES.PA_or_VA_bits = YES;
   ...
   */   
    ...
    if(getGATEALU == YES){
        ...
        if(getLDREG == YES){
            int getDRmux = GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION);
            if(getDRmux == NO){
                int register_num = CURRENT_LATCHES.IR & CondCode;
                register_num = register_num >> OFF9;
                NEXT_LATCHES.REGS[register_num] = Low16bits(BUS);
            }else{
        ...
        if(ldmdr == YES){
            if((CURRENT_LATCHES.MAR & READYBIT) == NO){
                ...
                NEXT_LATCHES.MDR = Low16bits(BUS & LowWord);
                int hold_value = Low16bits(BUS & LowWord);
                hold_value = hold_value << shiftby8;
                hold_value = Low16bits(hold_value);
                NEXT_LATCHES.MDR = NEXT_LATCHES.MDR + hold_value;
                CTLSIG[1] = YES;
            }
        }
    }
    else if(GetGATE_PC(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
        int getldreg = GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION);
        if(getldreg == YES){
            int getdrmux = GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION);
            if(getdrmux == NO){
        ...
        if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            NEXT_LATCHES.MAR = Low16bits(BUS);
            // must check for the next state for translation of state number of VM to PM
            NEXT_LATCHES.translate = NEXT_LATCHES.STATE_NUMBER;
            CTLSIG[0] = YES;
            if(NEXT_LATCHES.INTV != NO){
                intpt = TRUE;
                either = TRUE;
            ...
            // change from pa to va and vice versa
            if((NEXT_LATCHES.MAR & MASK15_9) >> rightShift9 < MAXFRAME){
                NEXT_LATCHES.STATE_NUMBER = firstVAPAState;
                int parse = 0;
                while(parse < CONTROL_STORE_BITS){
        ...
        if(GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION) && CTLSIG[1] != YES){
            NEXT_LATCHES.MDR = Low16bits(BUS);
        }
        
    }
    else if(GetGATE_MDR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
        if(GetLDPSR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            NEXT_LATCHES.ProgramStatusRegister = Low16bits(BUS);
            if((NEXT_LATCHES.ProgramStatusRegister & 0x8000) == 0x8000){
        ...
        if(GetLD_IR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            ...
            if((NEXT_LATCHES.IR & highWordModify) == 0xA000 || (NEXT_LATCHES.IR & highWordModify) == 0xB000){
                excpt = TRUE;
                either = TRUE;
        ...
        int getLDREG = GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION);
        if(getLDREG == YES){
            int getdrmuxx = GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION);
            if(getdrmuxx == NO){
                int register_num = CURRENT_LATCHES.IR & CondCode;
                register_num = register_num >> OFF9 ;
                NEXT_LATCHES.REGS[register_num] = Low16bits(BUS);
    ...
    else if(GetGATE_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
        if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            NEXT_LATCHES.MAR = Low16bits(BUS);
            NEXT_LATCHES.translate = NEXT_LATCHES.STATE_NUMBER;
            CTLSIG[0] = YES;
            ...
            // get rid of initial state because of VA to PA
            if((NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV)== PSR_PRIV && ((NEXT_LATCHES.MAR & MASK15_9) >> rightShift9) <= MAXFRAME && (NEXT_LATCHES.IR & HighWord) != highWordModify){
                excpt = TRUE;
                either = TRUE;
        ...
        if(GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            if(GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION) == NO){
                int dr_idx = CURRENT_LATCHES.IR & CondCode;
                dr_idx = dr_idx >> 9;
...
void latchSwitch_SSP(){
    int getswitchSSP = GetSWITCH_SSP(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getswitchSSP){
        NEXT_LATCHES.UserR6 = Low16bits(CURRENT_LATCHES.REGS[6]);
        NEXT_LATCHES.REGS[6] = Low16bits(CURRENT_LATCHES.SSP);
    }
}

void latchSwitch_USP(){
    int getswitchUSP = GetSWITCH_USP(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getswitchUSP){
        NEXT_LATCHES.SSP = Low16bits(CURRENT_LATCHES.REGS[6]);
        NEXT_LATCHES.REGS[6] = Low16bits(CURRENT_LATCHES.UserR6);
    }
}

void latchPCUpdate(){
    int getPCmux = GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    if(getPCmux == ADDRMODE){
        NEXT_LATCHES.PC = CURRENT_LATCHES.PC - nextLocation;
        CTLSIG[6] = YES;
    }

}

#define maskbits 0x01C0
#define inter 0xFFF8
#define maskBit2 0x0002
#define maskBit3 0x0004
#define maskBit1 0x0001
...
void latch_Addresses(){
    int ldVirtualAddr = GetLDVA(CURRENT_LATCHES.MICROINSTRUCTION);
    if(ldVirtualAddr){
        NEXT_LATCHES.VA = Low16bits(CURRENT_LATCHES.MAR);
    }
    int ldmarVA = GetLDREG_VA(CURRENT_LATCHES.MICROINSTRUCTION);
    if(ldmarVA){
        int jump = (NEXT_LATCHES.VA & MASK15_9) << YES;
        NEXT_LATCHES.MAR = Low16bits(pte + jump);
    }
    int PG_bits = GetPG_BITS(CURRENT_LATCHES.MICROINSTRUCTION);
    if(PG_bits){
        if(CURRENT_LATCHES.IVT == NO && (CURRENT_LATCHES.MDR & bit3) == bit3 && (CURRENT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV){
                excpt = YES;
                either = YES;
                NEXT_LATCHES.EXCV = BREN;
                NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
                ...
                NEXT_LATCHES.PA_or_VA_bits = NO;
                return;
        }
        else if(CURRENT_LATCHES.MDR & bit2){
                excpt = YES;
                either = YES;
                NEXT_LATCHES.EXCV = BREN;
                NEXT_LATCHES.STATE_NUMBER = excpt_STATE;
        ...
        NEXT_LATCHES.MDR = NEXT_LATCHES.MDR | bit0; 
        if(CURRENT_LATCHES.PA_or_VA_bits){
            NEXT_LATCHES.MDR = NEXT_LATCHES.MDR | bit1; 
        }
        NEXT_LATCHES.PA_or_VA_bits = NO;
    }
    int ldmarPA = GetLDREG_PA(CURRENT_LATCHES.MICROINSTRUCTION);
    if(ldmarPA){
        int jump = (CURRENT_LATCHES.VA & 511);
        int hold_value = (CURRENT_LATCHES.MDR & 15872);
        NEXT_LATCHES.MAR = Low16bits(hold_value + jump);
    }
}
...
void latch_datapath_values() {
    ...
    if(GetSWITCH_SSP(CURRENT_LATCHES.MICROINSTRUCTION)){
        NEXT_LATCHES.UserR6 = Low16bits(CURRENT_LATCHES.REGS[6]);
        NEXT_LATCHES.REGS[6] = Low16bits(CURRENT_LATCHES.SSP);
    }
    if(GetSWITCH_USP(CURRENT_LATCHES.MICROINSTRUCTION)){
        NEXT_LATCHES.SSP = Low16bits(CURRENT_LATCHES.REGS[6]);
        NEXT_LATCHES.REGS[6] = Low16bits(CURRENT_LATCHES.UserR6);
    }
    if(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == ADDRMODE){
        NEXT_LATCHES.PC = CURRENT_LATCHES.PC - nextLocation;
        CTLSIG[6] = YES;
    }
    if(GetLD_PC(CURRENT_LATCHES.MICROINSTRUCTION) == YES && CTLSIG[6] == NO){
        if(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == NO){
            NEXT_LATCHES.PC = NEXT_LATCHES.PC + nextLocation;
        }
        if(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
            int register_num = (CURRENT_LATCHES.IR & R7) >> OFF6;
            NEXT_LATCHES.PC = CURRENT_LATCHES.REGS[register_num];
            if(((NEXT_LATCHES.PC & MASK15_9) >> rightShift9) <= MAXFRAME && (NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV && CURRENT_LATCHES.IVT == NO){
                excpt = TRUE;
                either = TRUE;
                NEXT_LATCHES.EXCV = BREN; 
                NEXT_LATCHES.STATE_NUMBER = 36;
        ...
        if(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION) == nextLocation){ 
            int second_address = 0;
            int first_address = 0;
            ...
            switch(GetADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION)){
                ...
                case READYBIT:{
                    second_address = SEXT(CURRENT_LATCHES.IR & 0x003F, OFF6);
                    break;
                }
                case BREN:{
                    second_address = SEXT(CURRENT_LATCHES.IR & 0x01FF, OFF9);
                    break;
                }
                case ADDRMODE:{
                    second_address = SEXT(CURRENT_LATCHES.IR & 0x07FF, 11);
                    break;
                }
            }
            ...
            NEXT_LATCHES.PC = Low16bits(first_address + second_address);
            if(((NEXT_LATCHES.PC & MASK15_9) >> rightShift9) <= MAXFRAME && (NEXT_LATCHES.ProgramStatusRegister & PSR_PRIV) == PSR_PRIV && CURRENT_LATCHES.IVT == NO){
                excpt = TRUE;
                either = TRUE;
                NEXT_LATCHES.EXCV = BREN;
    ...
    CTLSIG[6] = NO;
    if(GetLD_BEN(CURRENT_LATCHES.MICROINSTRUCTION) == YES){
        latchBEN();
    }
    if(GetLD_CC(CURRENT_LATCHES.MICROINSTRUCTION)==1){
        ...
        if(NEXT_LATCHES.REGS[sourceReg] == NO){
            ...
            NEXT_LATCHES.N = n_bit;
            NEXT_LATCHES.P = p_bit;
            NEXT_LATCHES.ProgramStatusRegister = (CURRENT_LATCHES.ProgramStatusRegister & inter) | maskBit2;
        }
            ...
            NEXT_LATCHES.N = n_bit;
            NEXT_LATCHES.P = p_bit;
            NEXT_LATCHES.ProgramStatusRegister = (CURRENT_LATCHES.ProgramStatusRegister & inter) | maskBit3;
        }
            ...
            NEXT_LATCHES.N = n_bit;
            NEXT_LATCHES.P = p_bit;
            NEXT_LATCHES.ProgramStatusRegister = (CURRENT_LATCHES.ProgramStatusRegister & inter) | maskBit1;
        }
    }
}
